...
def build_matrix(log: Path, binary: Path) -> list[tuple[str, str, str]]:
    ...
    for applet in applet_list(binary):
        if applet in from_log:
            ...
            status, evidence = "unverified", "no-gate"
...
def main() -> int:
    ...
    if not args.log or not args.binary:
        ap.error("--log and --binary are required")
    ...
    if args.write_development:
        dev = Path(args.write_development)
        ...
        dev.write_text(
            ...
            "# make busybox-matrix; gaps are expected here on purpose.\n"
            + "\n".join(listed)
            + "\n"
        )
